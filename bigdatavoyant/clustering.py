...
def compute_clusters(pois, alg='dbscan', min_pts=None, eps=None, n_jobs=1):
    ...
    info = {
        ...
        'num_of_clustered_pois': len(pois_in_clusters),
        'num_of_outlier_pois': len(pois_noise)
    }

    print('Number of clusters: %d' % (info['num_of_clusters']))
    print('Number of clustered POIs: %d' % (info['num_of_clustered_pois']))
    print('Number of outlier POIs: %d' % (info['num_of_outlier_pois']))

    return pois_in_clusters, eps_per_cluster, info
